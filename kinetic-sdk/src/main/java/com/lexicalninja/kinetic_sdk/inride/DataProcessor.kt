...
class DataProcessor {
    ...
    private val cadenceBufferSize =
            CADENCE_BUFFER_SIZE_DEFAULT
    private val cadenceBufferWeight =
            CADENCE_BUFFER_WEIGHT_DEFAULT
    private var cadenceBufferCount = 0
    private val cadenceBuffer =
            arrayOfNulls<CadenceMark>(CADENCE_BUFFER_SIZE_MAX)
    ...
    private fun adjustCadence(crankRPM: Long, timestamp: Double): Double {
        ...
        cadenceBuffer[0]!!.cadenceRPM =
                adjustedRPM
        cadenceBuffer[0]!!.timestamp = timestamp
        cadenceBufferCount = Math.min(
                cadenceBufferSize,
                cadenceBufferCount + 1
        )
        var rollingRPM =
                adjustedRPM * cadenceBufferWeight
    ...
    private fun powerForSpeed(
        ...
        dragOffset = if (spindown > 0 && rawPower > 0) {
            val proFlywheel =
                    hasProFlywheel(spindown)
            val spindownTimeMS = spindown * 1000
            val dragOffsetSlope =
                    if (proFlywheel) PowerConstants.DragSlopeOffsetPro else PowerConstants.DragSlopeOffset
            val dragOffsetPowerSlope =
                    if (proFlywheel) PowerConstants.PowerSlopeOffsetPro else PowerConstants.PowerSlopeOffset
            val yIntercept =
                    if (proFlywheel) PowerConstants.YInterceptPro else PowerConstants.YIntercept
